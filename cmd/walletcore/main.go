...
import (
	...
	events "github.com.br/devfullcycle/fc-ms-wallet/pkg/events"
...
func main() {
	db, err := sql.Open("mysql", fmt.Sprintf("%s:%s@tcp(%s:%s)/%s?charset=utf8&parseTime=True&loc=Local",
		...
		"root",   // senha do MySQL
		"mysql",  // host do MySQL (ou "localhost")
		"3306",   // porta do MySQL (de acordo com a sua configuração no Docker Compose)
		"wallet", // nome do banco de dados
	))
	if err != nil {
		panic(err)
	}
	...
	webserver.Addhandler("/clients", clientHandler.Create)
	webserver.Addhandler("/accounts", accountHandler.Create)
	webserver.Addhandler("/transactions", transactionHandler.Create)
